...
## Исправленные тесты
### ✅ Оригинальный тест `tests/integration/test_burger_menu_links_playwright.py`:
- Увеличен таймаут с 1500ms до 5000ms
- Добавлена полноценная антибот защита (user-agent, заголовки, флаги запуска)
- Реализована гибкая настройка режима через переменные окружения TEST_HEADLESS
- Исправлена обработка дубликатов элементов через `.first()`
- Добавлена изоляция тестов через session-scoped fixtures

### ✅ Новый тест `tests/integration/test_burger_menu_isolated.py`:
- Полностью изолированные тесты через function-scoped fixtures
- Расширенная антибот защита
- Поддержка обоих режимов (headless/GUI)
