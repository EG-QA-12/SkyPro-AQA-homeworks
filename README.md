# BLL Tests: Фреймворк автоматизации тестирования

## 📚 Документация

- [docs/GETTING_STARTED.md](docs/GETTING_STARTED.md) - Начало работы
- [docs/FRAMEWORK_GUIDE.md](docs/FRAMEWORK_GUIDE.md) - Руководство по фреймворку
- [docs/RUNNING_TESTS_GUIDE.md](docs/RUNNING_TESTS_GUIDE.md) - Запуск тестов
- [docs/auth_api_reference.md](docs/auth_api_reference.md) - Справочник API системы авторизации
- [docs/TESTING/WRITING_TESTS.md](docs/TESTING/WRITING_TESTS.md) - Примеры написания тестов
- [docs/TESTING/BEST_PRACTICES.md](docs/TESTING/BEST_PRACTICES.md) - Лучшие практики тестирования
- [docs/guides/PLAYWRIGHT_MCP_TESTING_GUIDE.md](docs/guides/PLAYWRIGHT_MCP_TESTING_GUIDE.md) - Руководство по тестированию с Playwright
- [docs/guides/GIT_BRANCHING_STRATEGY.md](docs/guides/GIT_BRANCHING_STRATEGY.md) - Стратегия ветвления Git
- [docs/MIGRATION/MIGRATION_GUIDE.md](docs/MIGRATION/MIGRATION_GUIDE.md) - Миграция на новую систему авторизации

## 🚀 Быстрый старт с новым фреймворком

### ⚡ Самые быстрые команды:
```bash
# Быстрый запуск (15 секунд)
scripts/run_tests_quick.bat

# Параллельный запуск (20 секунд)
scripts/run_tests_parallel.bat

# Запуск примеров нового фреймворка
pytest tests/integration/test_new_framework_example.py -v
```

## 🏗️ Структура проекта

```
Bll_tests/                              # Корневая директория
├── framework/                          # Общий фреймворк автоматизации
│   ├── utils/                         # Общие утилиты (auth_utils, allure_utils)
│   └── app/                           # Специфичный код приложения
├── tests/                             # Тестовые директории
│   ├── smoke/                         # Smoke-тесты (критически важные проверки)
│   │   └── burger_menu/               # Тесты навигации бургер-меню
│   │       ├── left_column/           # Тесты левой колонки меню
│   │       ├── right_column/          # Тесты правой колонки меню
│   │       └── pages/                 # Page Object классы
│   ├── integration/                   # Интеграционные тесты (основная тестовая база)
│   │   ├── redirect_tests/            # Тесты редиректов (были перенесены из e2e)
│   │   ├── sso/                       # SSO-тесты (были перенесены из e2e)
│   │   └── infrastructure/            # Вспомогательные интеграционные тесты
│   └── e2e/                           # Инфраструктурные файлы, заглушки, настройки
├── scripts/                           # Общие скрипты
├── docs/                              # Документация
├── config/                            # Конфигурация фреймворка и секреты
│   ├── .env.template                  # Шаблон переменных окружения
│   ├── auth_config.json               # Конфигурация авторизации
│   ├── db_settings.py                 # Настройки базы данных
│   └── secrets_manager.py             # Менеджер секретов
└── cookies/                           # Файлы cookies (в .gitignore)
    ├── admin_cookies.json
    ├── user_cookies.json
    ├── moderator_cookies.json
    └── [другие cookie файлы]
```

## 🚀 Быстрый старт

### ⚡ Запуск тестов (самое быстрое)
```bash
# Windows - двойной клик по батнику:
scripts/run_tests_quick.bat        # ~15 сек, краткий вывод
scripts/run_tests_parallel.bat     # ~20 сек, подробный вывод

# Командная строка:
python -m pytest -n 10 -v          # Параллельно, 10 потоков
```

📖 **Полный гид**: [`scripts/QUICK_TESTING_GUIDE.md`](scripts/QUICK_TESTING_GUIDE.md)

### 1. Установка зависимостей
```bash
# Установка зависимостей фреймворка
pip install -r requirements.txt

# Установка браузеров для тестирования
playwright install
```

### 2. Запуск тестов
```bash
# Smoke-тесты (критически важные проверки)
pytest tests/smoke/ -v --alluredir=allure-results

# Интеграционные тесты (основная тестовая база)
pytest tests/integration/ -v --alluredir=allure-results

# Отчет Allure
allure serve allure-results
```

### 3. Настройка секретов
1. Скопируйте `config/.env.template` в `config/.env`
2. Заполните необходимые переменные окружения
3. Убедитесь, что файлы секретов не попадают в git (.gitignore)


## 🔧 Описание компонентов

### 📦 Framework (framework/)
**Основа фреймворка автоматизации**
- `utils/` - Переиспользуемые утилиты (авторизация, отчетность, безопасность)
- `app/` - Специфичная логика приложения

**Ключевые модули:**
- `auth_utils.py` - Авторизация и работа с пользователями
- `allure_utils.py` - Интеграция с Allure для красивых отчетов
- `cookie_constants.py` - Константы для работы с cookies

### 🧪 Tests (tests/)
**Тестовые сценарии**
- `smoke/` - Smoke-тесты (критически важные проверки)
  - `burger_menu/` — тесты навигации бургер-меню
    - `left_column/` — тесты левой колонки (быстрые, без задержек)
    - `right_column/` — тесты правой колонки (с визуальными паузами)
    - `pages/` — Page Object классы для бургер-меню
- `integration/` - Интеграционные тесты (основная тестовая база)
  - `redirect_tests/` — тесты редиректов
  - `sso/` — тесты SSO
  - `infrastructure/` — инфраструктурные интеграционные тесты
- `e2e/` - Инфраструктурные файлы, заглушки, настройки

### ⚙️ Config (config/)
**Конфигурационные файлы**
- Настройки pytest, allure, и браузеров

### 🔐 Безопасность
- `config/` - Конфигурация и секретные данные (переменные окружения, настройки)
- `cookies/` - Файлы сессий (автоматически игнорируются git)

## 📋 Лучшие практики

### Рекомендации по лучшим практикам:
1. **Безопасность превыше всего**: Никогда не коммитьте пароли и секреты
2. **Читаемость кода**: Используйте понятные имена переменных и функций
3. **Документирование**: Всегда добавляйте docstrings к новым функциям
4. **Тестирование**: Один тест = одна проверка
5. **Структура**: Следуйте существующей архитектуре проекта
6. **Авторизация**: Используйте встроенные механизмы авторизации фреймворка

В случае вопросов или неполадок, начните с чтения документации в каждой ключевой папке!
---
_Эта документация отражает реальную структуру проекта и может быть обновлена в будущем._

_Для доступной работы с проектом рекомендуется ознакомиться с README в каждой ключевой папке!_
