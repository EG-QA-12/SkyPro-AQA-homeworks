...
Все интеграционные тесты используют умную авторизацию через `SmartAuthManager`.
Перед запуском тестов убедитесь, что:
1. Настроены переменные окружения в `.env` файле
2. Наличие валидных куки авторизации
3. Доступ к серверу `https://expert.bll.by`
