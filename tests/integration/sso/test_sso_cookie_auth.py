...
def test_sso_cookie_auth_all_domains(
    ...
    Выполняет комплексную проверку единого входа (SSO) для всех доменов
    экосистемы Bll. Тест использует HTTP-запросы для проверки работы
    куки авторизации test_joint_session без необходимости запуска браузера.
    
    Сценарий теста:
    1. GET запрос без кук → проверка неавторизованного состояния
    2. Установка кук пользователя → GET запрос с куками → проверка авторизованного состояния
    3. Сравнение HTML-ответов для валидации изменения состояния авторизации
    ...
    Args:
        isolated_sso_client: Изолированная HTTP сессия для тестирования,
            гарантирующая отсутствие влияния между тестами
        random_user_cookies: Словарь с данными кук случайного пользователя,
            включая имя пользователя и коллекцию кук
        domain_url: URL домена для тестирования (параметризовано через pytest.mark.parametrize)
        
    Returns:
        None: Тест использует assert для проверки условий
        
    Raises:
        AssertionError: при неудачной авторизации или проблемах с изоляцией тестов
    """
...
def test_sso_requests_isolation(isolated_sso_client: SSORequestsClient) -> None:
    ...
    Убеждается что каждый тест получает чистую HTTP сессию без кук
    от предыдущих тестов. Это критически важно для надежности тестов,
    так как предотвращает ложные срабатывания из-за "загрязнения" сессии.
    
    Тест выполняет две проверки:
    1. Проверка отсутствия кук в новой сессии
    2. Проверка неавторизованного состояния при запросе к домену
    
    Args:
        isolated_sso_client: Изолированная HTTP сессия, созданная
            специально для этого теста, с гарантией изоляции
        
    Returns:
        None: Тест использует assert для проверки условий
        
    Raises:
        AssertionError: при обнаружении кук в новой сессии
    """
...
def test_sso_cookies_availability(sso_test_info: Dict[str, Any]) -> None:
    ...
    Проверяет что файлы кук существуют в файловой системе и содержат
    корректные данные для тестирования SSO. Это предварительная проверка,
    которая помогает выявить проблемы с инфраструктурой тестирования
    до запуска основных тестов.
    
    Тест проверяет:
    - Наличие директории с файлами кук
    - Наличие хотя бы одного файла кук
    - Корректность конфигурации тестирования (таймауты, имена кук)
    
    Args:
        sso_test_info: Словарь с информацией о конфигурации SSO тестирования,
            включающий количество пользователей, список доступных пользователей,
            наличие директории с куками, таймауты и ожидаемые имена кук
            
    Returns:
        None: Тест использует assert для проверки условий
        
    Raises:
        AssertionError: при отсутствии директории с куками или
            недостаточном количестве файлов кук
    """
