...
from framework.utils.auth_utils import (
...
def test_cookie_auth_on_domain(
    isolated_context: Tuple[BrowserContext, Page],
    url: str
) -> None:
    """
    ...
    with allure.step("Шаг 1: Проверка отсутствия авторизации"):
        page.goto(url)
        assert is_guest(page), "Ожидалось, что пользователь не авторизован, но найдены признаки авторизации"

    with allure.step("Шаг 2: Добавление авторизационных cookies"):
        cookies = get_random_user_cookie(context)
        if not cookies:
            pytest.fail("Не удалось получить валидные cookies")
        context.add_cookies(cookies)

    with allure.step("Шаг 3: Проверка наличия авторизации после добавления cookies"):
        page.reload()
