"""
Burger Menu Params New - Multi-Domain Parameterized Tests.

Обновленная конфигурация для тестирования burger menu на всех доменах системы.
Использует параметризацию для запуска тестов на 5 доменах одновременно.
Включает автоматическую диагностику через playwright_allure fixture.
"""
...
def multi_domain_context(request):
    ...
    Добавляет allow-session параметр для обхода защиты от ботов
    в headless режиме.
...
def smart_authenticated_context(browser):
    ...
    if session_cookie:
        ...
        print("✅ Используется валидная кука для роли 'admin'")
    else:
