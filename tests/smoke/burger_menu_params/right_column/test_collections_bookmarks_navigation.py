...
class TestCollectionsBookmarksNavigationParams:
    ...
    def test_collections_bookmarks_navigation(self, multi_domain_context, domain_aware_authenticated_context):
        ...
        try:
            ...
            burger_menu.click_link_by_text("Подборки и закладки")

            # Check the final URL (with redirects followed)
            current_url = page.url
            print(f"Текущий URL: {current_url}")  # Для отладки

            # Collections and bookmarks should stay on the same domain
