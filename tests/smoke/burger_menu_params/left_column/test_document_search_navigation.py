"""
Burger Menu Left Column - Document Search Navigation - Multi-Domain Parameterized Tests.
...
class TestDocumentSearchNavigationParams:
    @pytest.mark.parametrize('multi_domain_context',
                            ['bll', 'expert', 'bonus', 'ca', 'cp'],
                            indirect=True,
                            ids=['Main(bll.by)', 'Expert', 'Bonus', 'CA', 'CP'])
    def test_document_search_navigation(self, multi_domain_context, domain_aware_authenticated_context):
        ...
        try:
            ...
            assert "docs" in current_url.lower() and "bll.by" in current_url.lower(), \
                f"URL не содержит docs и bll.by: {current_url}"
        finally:
            page.close()
