"""
Burger Menu Cross-Domain Navigation - Moderator Messages - Expert Domain.
...
class TestModeratorMessagesNavigationParams:
    @pytest.mark.parametrize('multi_domain_context',
                            ['bll', 'expert', 'bonus', 'ca', 'cp'],
                            indirect=True,
                            ids=['Main(bll.by)', 'Expert', 'Bonus', 'CA', 'CP'])
    def test_moderator_messages_navigation(self, multi_domain_context, domain_aware_authenticated_context):
        ...
        try:
            ...
            assert ("expert.bll.by" in current_url.lower() or
                    "ca.bll.by/user/profile" in current_url.lower()), \
                f"URL не содержит expert.bll.by или ca.bll.by/user/profile: {current_url}"
        finally:
            page.close()
