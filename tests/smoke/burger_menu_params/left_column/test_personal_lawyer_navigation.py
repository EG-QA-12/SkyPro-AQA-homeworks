"""
Burger Menu Left Column - Personal Lawyer Navigation - Multi-Domain Parameterized Tests.
...
class TestPersonalLawyerNavigationParams:
    @pytest.mark.parametrize('multi_domain_context',
                            ['bll', 'expert', 'bonus', 'ca', 'cp'],
                            indirect=True,
                            ids=['Main(bll.by)', 'Expert', 'Bonus', 'CA', 'CP'])
    def test_personal_lawyer_navigation(self, multi_domain_context, domain_aware_authenticated_context):
        ...
        try:
            ...
            assert ("lichnyj-yurist-206044" in current_url.lower() or
                    "ca.bll.by/user/profile" in current_url.lower()), \
                f"URL не содержит lichnyj-yurist-206044 или ca.bll.by/user/profile: {current_url}"
        finally:
            page.close()
