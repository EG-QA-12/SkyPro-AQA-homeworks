"""
Burger Menu Cross-Domain Navigation - My Questions and Answers - Expert Domain.
...
class TestMyQuestionsAnswersNavigationParams:
    @pytest.mark.parametrize('multi_domain_context',['bll', 'expert', 'bonus', 'ca', 'cp'], indirect=True, ids=['Main(bll.by)', 'Expert', 'Bonus', 'CA', 'CP'])
    def test_my_questions_answers_navigation(self, multi_domain_context, domain_aware_authenticated_context):
        ...
        try:
            ...
            assert re.search(r'expert\.bll\.by', current_url), \
                f"URL не содержит паттерн домена expert.bll.by: {current_url}"        finally:
            page.close()
