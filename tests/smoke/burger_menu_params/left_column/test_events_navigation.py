"""
Burger Menu Left Column - Events Navigation - Multi-Domain Parameterized Tests.
...
class TestEventsNavigationParams:
    @pytest.mark.parametrize('multi_domain_context',
                            ['bll', 'expert', 'bonus', 'ca', 'cp'],
                            indirect=True,
                            ids=['Main(bll.by)', 'Expert', 'Bonus', 'CA', 'CP'])
    def test_events_navigation(self, multi_domain_context, domain_aware_authenticated_context):
        ...
        try:
            ...
            assert "kalendar-meropriyatij-471630" in current_url.lower(), \
                f"URL не содержит kalendar-meropriyatij-471630: {current_url}"
        finally:
            page.close()
