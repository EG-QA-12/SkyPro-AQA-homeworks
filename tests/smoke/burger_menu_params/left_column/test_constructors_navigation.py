"""
Burger Menu Left Column - Constructors Navigation - Multi-Domain Parameterized Tests.
...
class TestConstructorsNavigationParams:
    @pytest.mark.parametrize('multi_domain_context',['bll', 'expert', 'bonus', 'ca', 'cp'], indirect=True, ids=['Main(bll.by)', 'Expert', 'Bonus', 'CA', 'CP'])
    def test_constructors_navigation(self, multi_domain_context, domain_aware_authenticated_context):
        ...
        try:
            ...
            # URL contains constructor ID for all domains
            # Check URL pattern with regex (ignores query parameters)
            assert re.search(r'konstruktory-200077', current_url), \
                f"URL не содержит паттерн конструкторов konstruktory-200077: {current_url}"        finally:
            page.close()
