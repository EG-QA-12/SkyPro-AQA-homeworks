...
class TestProcurementNavigationParams:
    ...
    def test_procurement_navigation(self, multi_domain_context, domain_aware_authenticated_context):
        ...
        try:
            ...
            # Check URL pattern (should redirect to gz.bll.by or ca.bll.by/user/profile)
            assert ("gz.bll.by" in current_url.lower() or
                    "ca.bll.by/user/profile" in current_url.lower()), \
                f"URL не содержит gz.bll.by или ca.bll.by/user/profile: {current_url}"
        finally:
            page.close()
