...
Новый фреймворк представляет собой современное решение для автоматизации тестирования, 
построенное на принципах SOLID и обеспечивающее высокую читаемость, 
поддерживаемость и расширяемость тестов.
...
- Интеллектуальное кэширование кук
- Поддержка нескольких источников получения кук
...
Новый фреймворк включает в себя продвинутую систему авторизации, которая обеспечивает:
- **Автоматическую авторизацию** для всех тестов без необходимости ручной настройки
- **Поддержку различных ролей** пользователей (admin, moderator, user)
- **Безопасное хранение учетных данных** в переменных окружения и конфигурационных файлах
- **Автоматическое обновление сессий** при истечении срока действия кук
- **Интеллектуальное кэширование кук** для повышения производительности
- **Поддержку нескольких источников получения кук** (ENV, файлы, API-логин)

#### Источники получения кук
Система авторизации поддерживает три источника получения кук в порядке приоритета:
...
Этот порядок соответствует принципам Управления Окружением и Изоляции: сначала
используются явные конфиги, затем артефакты прошлых запусков, и только в крайнем случае
выполняется сетевой логин.

#### Примеры использования
```python
# В базовом классе тестов автоматически доступны клиенты с разными ролями:
# self.admin_client - клиент администратора (уже авторизован)
# self.moder_client - клиент модератора (уже авторизован)
# self.user_client - клиент обычного пользователя (уже авторизован)

def test_with_admin_role(self):
    # Клиент уже авторизован, можно сразу использовать
    questions = self.admin_client.get_moderation_panel_data()
...
- Используйте встроенные клиенты (`self.admin_client`, `self.moder_client`, `self.user_client`)
- Не создавайте клиенты вручную - это может привести к проблемам с авторизацией
- При необходимости используйте фикстуры авторизации (`admin_client`, `session_cookie`)
- Для тестирования разных ролей используйте параметризацию с `UserRole` enum
