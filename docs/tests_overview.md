...
## Новые тесты: Cross-Domain Cookie Authorization

Эти тесты проверяют работу авторизационных cookies на разных поддоменах приложения. Они обеспечивают, что сессия сохраняется при переходе между доменами.

- Локация: `tests/e2e/auth/test_cross_domain_cookie_auth.py`
- Ключевые особенности:
  - Параметризация по URL
  - Изоляция через isolated_context
  - Проверки на гостя и авторизованного пользователя

Пример запуска: `pytest tests/e2e/auth/test_cross_domain_cookie_auth.py -v` 