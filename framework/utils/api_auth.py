...
from .db_helpers import update_user_in_db
...
class APIAuthManager:
    ...
    def login_user(self, username: str, password: str) -> AuthResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if session_cookies and COOKIE_NAME in session_cookies:
    ...
    def _extract_cookies(self, response: requests.Response) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        Returns:
            Словарь с куками в формате совместимом с Playwright
        """
        try:
            cookies_dict = {}
    ...
    def mass_authorize_users(self, users: List[Dict[str, str]], save_to_files: bool = True, 
                           update_database: bool = True) -> Tuple[List[AuthResult], Dict[str, Any]]:
        """
        Массовая авторизация списка пользователей через API.
        ...
        """
        logger.info(f"🚀 Начинаем массовую API авторизацию {len(users)} пользователей")
        ...
        for i, user in enumerate(users, 1):
            ...
            print(f"\n📡 [{i}/{len(users)}] API авторизация: {username}")
            
            # Выполняем авторизацию
            auth_result = self.login_user(username, password)
            results.append(auth_result)
            
            if auth_result.success:
                successful_auths += 1
                print(f"   ✅ Успешно авторизован через API")
                ...
                if save_to_files and auth_result.cookies:
                    try:
                        ...
                        print(f"   💾 Куки сохранены в файл: {cookie_file}")
                    except Exception as e:
                ...
                if update_database and auth_result.session_token:
                    try:
                        update_user_in_db(username, auth_result.session_token)
                        print(f"   🗄️  База данных обновлена")
                    except Exception as e:
                        logger.error(f"Ошибка обновления БД для {username}: {str(e)}")
                        
            else:
                failed_auths += 1
                print(f"   ❌ Ошибка: {auth_result.error_message}")
        ...
        print("\n" + "="*80)
        print("📊 СТАТИСТИКА API АВТОРИЗАЦИИ")
        print("="*80)
        print(f"👥 Всего пользователей: {stats['total_users']}")
        print(f"✅ Успешно авторизовано: {stats['successful']}")
        print(f"❌ Неудачных авторизаций: {stats['failed']}")
        print(f"📈 Процент успеха: {stats['success_rate']:.1f}%")
        print(f"⏱️  Общее время: {stats['elapsed_time']:.2f} сек")
        print(f"⚡ Среднее время на пользователя: {stats['avg_time_per_user']:.2f} сек")
        print("="*80)
        
        logger.info(f"Массовая авторизация завершена: {successful_auths}/{len(users)} успешно")
...
def api_mass_auth(users: List[Dict[str, str]], save_files: bool = True, 
                  update_db: bool = True) -> Tuple[List[AuthResult], Dict[str, Any]]:
    """
    Функция-обертка для быстрой массовой API авторизации.
    ...
    try:
        return auth_manager.mass_authorize_users(users, save_files, update_db)
    finally:
        auth_manager.close()


if __name__ == "__main__":
    print("Модуль API авторизации для быстрой массовой авторизации пользователей")
    print("Использование:")
    print("from framework.utils.api_auth import api_mass_auth")
    print("results, stats = api_mass_auth(users_list)") 