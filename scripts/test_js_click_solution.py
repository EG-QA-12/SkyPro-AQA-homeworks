#!/usr/bin/env python3
"""
Решение для клика по скрытым элементам правой колонки через JavaScript.
"""
import sys
import os
...
def test_js_click_solution():
    """Тест решения для клика по скрытым элементам правой колонки."""
    print("🔍 Тест решения для клика по скрытым элементам правой колонки")
    print("=" * 70)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            right_column_elements = [
                ...
                {"name": "Бонусы", "expected_url": "https://bonus.bii.by/bonus#newtask"},
            ...
            for element_info in right_column_elements:
                ...
                try:
                    # Используем прямой JavaScript клик для скрытых элементов
                    js_click_result = page.evaluate(f"""
                        ...
                        for (let elem of elements) {{
                            ...
                            if (text.includes('{element_name}') || href.includes('{expected_url.split('//')[1].split('/')[0]}')) {{
                                found_element = elem;
            ...
            # Проверяем, что хотя бы один элемент работает
            if success_count > 0:
                print(f"\n🎉 УРА! {success_count} элементов успешно работают через JavaScript клик!")
                print("   Это доказывает, что решение работает!")
                return True
            else:
                print(f"\n😞 Ни один элемент не сработал через JavaScript клик")
                return False
...
if __name__ == "__main__":
    success = test_js_click_solution()
